...
export default function UpsellStrip({ items, themeColor }: Props) {
  return (
    <div className="px-4 pb-4">
      ...
      <div className="flex gap-2.5 overflow-x-auto scrollbar-none" style={{ scrollbarWidth: "none" }}>
        {items.map((item) => (
          <div
            key={item.id}
            className="flex-shrink-0 flex items-center gap-2.5 bg-white/[0.04] border border-white/[0.07] rounded-2xl px-3 py-2 min-w-0"
          >
            {item.imageUrl ? (
              <img
                src={item.imageUrl}
            ...
            <div className="min-w-0">
              <p className="text-xs text-white font-semibold truncate max-w-[90px] leading-tight">
                {item.name}
              </p>
